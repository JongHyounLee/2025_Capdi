...
#endif
volatile uint8_t uart4_dma_done = 0;
...
//
//    HAL_StatusTypeDef st = HAL_UART_Transmit_DMA(&huart4, (uint8_t*)data, len);
//    if (st != HAL_OK) {
...
void vTaskLogger(void *pvParameters)
{
    for (;;)
    {
        ...
        SCB_CleanDCache();   // 가장 단순한 방법 (전체 D-Cache flush)
        while (uartDmaBusy) {
            vTaskDelay(pdMS_TO_TICKS(1));
        }

        uartDmaBusy = 1;
        HAL_UART_Transmit_DMA(&huart5, (uint8_t *)msg, len);
    }
}
...
int main(void)
{
  ...
  //UART4_DMA_TestStart();
   ...
   xTaskCreate(vTaskLogger,"vTaskLogger",2048, NULL,3, NULL);
...
uint8_t uart4_dma_msg[] = "UART4 DMA test: Hello from STM32H743!\r\n";
...
void UART4_DMA_TestStart(void)
{
    ...
    if (HAL_UART_Transmit_DMA(&huart5,
                              uart4_dma_msg,
                              sizeof(uart4_dma_msg) - 1) != HAL_OK)
    {
        // DMA 전송 시작 자체가 실패하면 여기로 옴
        Error_Handler();
    }
}
